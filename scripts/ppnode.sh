...
install() {
    bash <(curl -Ls "${PPNODE_RAW_BASE}/scripts/install.sh")
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls "${PPNODE_RAW_BASE}/scripts/install.sh") $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/ppnode -N --no-check-certificate "${PPNODE_RAW_BASE}/scripts/ppnode.sh"
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/${PPNODE_GITHUB_REPO} ---
  ${green}0.${plain} 修改配置
————————————————
